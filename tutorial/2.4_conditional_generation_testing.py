...
# plm, tokenizer, model_config, WrapperClass = load_plm("t5", "t5-base") # you can change to t5, with the rest unchanged.
...
mytemplate = PrefixTuningTemplatePro(model=plm,  tokenizer=tokenizer, text=' {"placeholder":"text_a"} <|endoftext|> {"mask"} ', using_decoder_past_key_values=False)
...
def evaluate(prompt_model, dataloader):
    ...
    for step, inputs in enumerate(dataloader):
        if use_cuda:
            inputs = inputs.cuda()
        # inputs["input_ids"] = inputs["input_ids"][:,:max(inputs['input_ids_len'])]
        # inputs["loss_ids"] = inputs["loss_ids"][:,:max(inputs['input_ids_len'])]
...
with open("../../Generated_sentence_webnlg_gpt2.txt",'w') as f:
