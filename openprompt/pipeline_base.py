...
class PromptClassificationAsGeneration(nn.Module, GenerationMixin):
    r'''``PromptModel`` with generation loss caculation, but a classification objective. 
    As unifying tasks as generation tasks has become a trend (See `paper <https://arxiv.org/abs/2103.10360>`)
    We provide a interface to directly train the classification tasks by filling the masked position with the label name
    (label words). When in prediction, only generating the whole desired target string will be seen as a correct prediction. 
